"""CRUD for custom_prompt draft / publish / rollback."""
...
from apps.custom_prompt_version.models import (
    CustomPromptDraftSave,
    CustomPromptVersion,
    CustomPromptVersionInfo,
    CustomPromptVersioningState,
)
from common.core.deps import Trans


def _to_info(
    row: CustomPromptVersion,
    *,
    published_id: Optional[int],
    draft_id: Optional[int],
) -> CustomPromptVersionInfo:
    return CustomPromptVersionInfo(
        id=row.id,
...
def _get_prompt_row(session: Session, prompt_id: int, oid: int) -> dict[str, Any]:
    row = session.execute(
        text(
            """
            SELECT id, oid, published_version_id, draft_version_id, prompt
            FROM custom_prompt
    ...
    if not row:
        raise Exception('custom_prompt_not_found')
    return dict(row)


def _next_version_no(session: Session, prompt_id: int) -> int:
    current = session.execute(
        text('SELECT COALESCE(MAX(version_no), 0) FROM custom_prompt_version WHERE prompt_id = :pid'),
        {'pid': prompt_id},
    ).scalar()
    return int(current or 0) + 1


def get_versioning_state(session: Session, prompt_id: int, oid: int) -> CustomPromptVersioningState:
    row = _get_prompt_row(session, prompt_id, oid)
    published_id = row.get('published_version_id')
    draft_id = row.get('draft_version_id')
    versions = session.execute(
        select(CustomPromptVersion)
        .where(CustomPromptVersion.prompt_id == prompt_id)
        .order_by(CustomPromptVersion.version_no.desc())
    ).scalars().all()
    ...
    return CustomPromptVersioningState(
        prompt_id=prompt_id,
...
def save_draft(
    ...
    if draft_id:
        draft = session.get(CustomPromptVersion, draft_id)
    ...
    # No draft pointer: create a new version as draft (v1 for brand-new prompts)
    ...
    session.flush()
    session.execute(
        text('UPDATE custom_prompt SET draft_version_id = :vid WHERE id = :pid'),
        {'vid': version.id, 'pid': prompt_id},
    )
    session.commit()
...
def publish_draft(
    ...
    if not draft_id:
        published_id = row.get('published_version_id')
        if published_id:
            published = session.get(CustomPromptVersion, published_id)
            if published:
    ...
    session.execute(
        ...
        {'vid': draft.id, 'prompt': draft.prompt, 'pid': prompt_id},
    )
    session.commit()
    session.refresh(draft)
...
def publish_version(
    ...
    """Rollback / set a historical version as published."""
    _get_prompt_row(session, prompt_id, oid)
    version = session.get(CustomPromptVersion, version_id)
    ...
    session.execute(
        ...
        {'vid': version.id, 'prompt': version.prompt, 'pid': prompt_id},
    )
    session.commit()
    row = _get_prompt_row(session, prompt_id, oid)
...
def list_versions(session: Session, prompt_id: int, oid: int) -> list[CustomPromptVersionInfo]:
