...
from apps.chat.clarification_util import apply_user_resolution, is_time_factor
from apps.chat.time_config import get_query_earliest_date
from apps.chat.time_i18n import resolve_lang_key
from apps.chat.time_range_prefilter import validate_time_free_text, validate_time_selection
from apps.chat.curd.chat import delete_chat_with_user, get_chart_data_with_user, get_chat_predict_data_with_user, \
    ...
    rename_chat_with_user, get_chat_log_history, get_chart_data_with_user_live, abandon_clarification, \
    is_clarification_pending, save_clarification_state
from apps.chat.models.chat_model import CreateChat, ChatRecord, RenameChat, ChatQuestion, AxisObj, QuickCommand, \
    ChatInfo, Chat, ChatFinishStep, ClarifyRequest
...
def _abandon_pending_clarification(session: SessionDep, chat_id: int):
    stmt = select(ChatRecord.id).where(
        and_(ChatRecord.chat_id == chat_id, ChatRecord.first_chat == False)).order_by(
        ChatRecord.create_time.desc()).limit(1)
    row = session.execute(stmt).fetchone()
    if row:
        record = get_chat_record_by_id(session, row[0])
        if is_clarification_pending(record):
            abandon_clarification(session, record.id)
...
async def stream_sql(session: SessionDep, current_user: CurrentUser, request_question: ChatQuestion,
                     ...
                     return_img: bool = True):
    try:
        if request_question.new_question:
            _abandon_pending_clarification(session, request_question.chat_id)

        llm_service = await LLMService.create(session, current_user, request_question, current_assistant,
                                              embedding=embedding)
        resume_sse_payload = None
        if request_question.resume_record_id:
            record = get_chat_record_by_id(session, request_question.resume_record_id)
            if record.chat_id != request_question.chat_id:
                raise Exception('Record does not belong to this chat')
            clarification = record.clarification if isinstance(record.clarification, dict) else {}
            if request_question.clarification_selection or request_question.clarification_free_text:
                earliest = get_query_earliest_date(session)
                current = clarification.get('current') or {}
                if is_time_factor(current):
                    lang_key = resolve_lang_key(current_user.language)
                    if request_question.clarification_selection:
                        err = validate_time_selection(
                            request_question.clarification_selection, earliest, lang=lang_key,
                        )
                        if err:
                            raise HTTPException(status_code=400, detail=err)
                    elif request_question.clarification_free_text:
                        err = validate_time_free_text(
                            request_question.clarification_free_text, earliest, lang=lang_key,
                        )
                        if err:
                            raise HTTPException(status_code=400, detail=err)
                clarification = apply_user_resolution(
                    clarification,
                    selection=request_question.clarification_selection,
                    free_text=request_question.clarification_free_text,
                    earliest_date=earliest,
                    lang=resolve_lang_key(current_user.language),
                )
                record = save_clarification_state(session, record.id, clarification)
            # Snapshot for SSE: ORM instance detaches after request session closes.
            clar_snapshot = record.clarification if isinstance(record.clarification, dict) else {}
            if clar_snapshot.get('resolved') and not clar_snapshot.get('current'):
                resume_sse_payload = {
                    'record_id': record.id,
                    'content': clar_snapshot,
                }
            llm_service.set_record(record)
            SQLBotLogUtil.info(
                f'clarify resume record={record.id} resolved={len(clar_snapshot.get("resolved") or [])}'
            )
        else:
    ...
    if stream:
        def _stream_with_clarification_update():
            if resume_sse_payload:
                yield 'data:' + orjson.dumps({
                    'type': 'clarification-resolved',
                    'content': resume_sse_payload['content'],
                    'record_id': resume_sse_payload['record_id'],
                }).decode() + '\n\n'
            yield from llm_service.await_result()

        return StreamingResponse(_stream_with_clarification_update(), media_type="text/event-stream")
    else:
...
@router.post("/record/{chat_record_id}/clarify", summary=f"{PLACEHOLDER_PREFIX}clarify_record")
async def clarify_record(session: SessionDep, current_user: CurrentUser, chat_record_id: int,
                         body: ClarifyRequest, current_assistant: CurrentAssistant):
    record = get_chat_record_by_id(session, chat_record_id)
    if record.create_by != current_user.id:
        raise HTTPException(status_code=403, detail='Forbidden')
    if body.abandon or body.new_question:
        abandon_clarification(session, chat_record_id)
        return {'abandoned': True}
    if not is_clarification_pending(record):
        raise HTTPException(status_code=400, detail='No pending clarification for this record')

    request_question = ChatQuestion(
        chat_id=record.chat_id,
        question=record.question,
        resume_record_id=record.id,
        clarification_selection=body.selection,
        clarification_free_text=body.free_text,
    )
