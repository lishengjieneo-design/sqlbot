...
export const PLATFORM_CLIENT_TYPES = ['dingtalk', 'lark', 'larksuite'] as const
export type PlatformClientType = (typeof PLATFORM_CLIENT_TYPES)[number]

const parseClientFromPath = (path: string): string | null => {
  if (!path || !path.includes('client=')) {
    return null
  }
  try {
    const query = path.includes('?') ? path.slice(path.indexOf('?') + 1) : ''
    const client = new URLSearchParams(query).get('client')
    if (client && PLATFORM_CLIENT_TYPES.includes(client as PlatformClientType)) {
      return client
    }
  } catch {
    return null
...
export const getPlatformClientType = (): PlatformClientType | null => {
  const fromSearch = getQueryString('client')
  if (fromSearch && PLATFORM_CLIENT_TYPES.includes(fromSearch as PlatformClientType)) {
    return fromSearch as PlatformClientType
  }
  const redirect = getQueryString('redirect')
  if (redirect) {
    const fromRedirect = parseClientFromPath(redirect)
    if (fromRedirect) {
      return fromRedirect as PlatformClientType
    }
  }
  return null
}
...
export const toLoginPage = (fullPath: string) => {
  ...
  const client = getQueryString('client') || parseClientFromPath(fullPath)
...
/** Preserve ?client= for Feishu/Lark in-app auto login when redirecting unauthenticated users. */
export const buildPlatformLoginRoute = (to: { fullPath?: string; path?: string; query?: Record<string, unknown> }) => {
  const fullPath = to.fullPath || to.path || '/'
  const client =
    (typeof to.query?.client === 'string' ? to.query.client : null) ||
    getQueryString('client') ||
    parseClientFromPath(fullPath)
  if (client && PLATFORM_CLIENT_TYPES.includes(client as PlatformClientType)) {
    const query: Record<string, string> = { client }
    const corpid =
      (typeof to.query?.corpid === 'string' ? to.query.corpid : null) || getQueryString('corpid')
    if (corpid) {
      query.corpid = corpid
    }
