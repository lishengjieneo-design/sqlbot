"""Models for extra_prompt versioning."""
...
class ExtraPromptVersion(SQLModel, table=True):
    __tablename__ = 'extra_prompt_version'
...
class ExtraPromptVersionInfo(BaseModel):
...
class ExtraPromptDraftSave(BaseModel):
    prompt: str
    change_note: Optional[str] = None


class ExtraPromptVersioningState(BaseModel):
    prompt_id: int
    published_version_id: Optional[int] = None
    draft_version_id: Optional[int] = None
    published: Optional[ExtraPromptVersionInfo] = None
    draft: Optional[ExtraPromptVersionInfo] = None
    versions: List[ExtraPromptVersionInfo] = Field(default_factory=list)
