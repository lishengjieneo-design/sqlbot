...
const larksuiteClientRequest = async () => {
  ...
  const finishWithCode = (code: string) => {
    ...
    toUrl(`?code=${encodeURIComponent(code)}&state=${encodeURIComponent(state)}`)
  }
    ...
    window['h5sdk'].ready(() => {
      window['tt'].requestAuthCode({
        ...
        success(res: any) {
          ...
          if (!code) {
            ElMessage.error('missing auth code from larksuite')
            return
          }
          finishWithCode(String(code))
        },
        fail(error: any) {
          const { errno, errString } = error || {}
          ElMessage.error(`error code: ${errno}, error msg: ${errString}`)
  ...
  // Newer Lark/Feishu clients support requestAccess; fall back to requestAuthCode.
  if (window['tt'].requestAccess) {
    window['h5sdk'].ready(() => {
      window['tt'].requestAccess({
        appID: clientId,
...
const toUrl = (url: string) => {
  ...
  const routeRedirect = new URLSearchParams(window.location.search).get('redirect')
  const hashTarget = redirect?.includes('chatPreview') ? `#${redirect}` : ''
  const search = url.startsWith('?') ? url : `?${url}`
  const nextSearch = new URLSearchParams(search.slice(1))
  if (routeRedirect && !nextSearch.has('redirect')) {
    nextSearch.set('redirect', routeRedirect)
  }
  window.location.href = `${origin}${pathname}?${nextSearch.toString()}${hashTarget}`
}
