...
def get_chat_record_by_id(session: SessionDep, record_id: int) -> ChatRecord | None:
    stmt = select(ChatRecord).where(and_(ChatRecord.id == record_id))
...
def is_clarification_pending(record: ChatRecord) -> bool:
    if not record or record.clarification_abandoned:
        return False
    if record.clarification_resolved:
        return False
    clarification = record.clarification
    if not clarification or not isinstance(clarification, dict):
        return False
    if clarification.get('abandoned') or clarification.get('force_finalized'):
        return False
    return bool(clarification.get('current'))


def save_clarification_state(session: SessionDep, record_id: int, clarification: dict,
                             resolved: bool = False, abandoned: bool = False) -> ChatRecord:
    stmt = update(ChatRecord).where(and_(ChatRecord.id == record_id)).values(
        clarification=clarification,
        clarification_resolved=resolved,
        clarification_abandoned=abandoned,
    )
    session.execute(stmt)
    session.commit()
...
def abandon_clarification(session: SessionDep, record_id: int) -> ChatRecord:
    record = get_chat_record_by_id(session, record_id)
    clarification = record.clarification if isinstance(record.clarification, dict) else {}
    clarification = {**clarification, 'abandoned': True, 'current': None}
    return save_clarification_state(session, record_id, clarification, resolved=False, abandoned=True)
