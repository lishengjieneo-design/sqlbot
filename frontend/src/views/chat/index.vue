    ...
    <el-container :loading="loading">
      ...
      >
        <div class="footer-stack">
          <ClarificationBlock
            v-if="pendingClarificationRecord"
            :record="pendingClarificationRecord"
            @select="onClarificationSelect"
            @new-question="onClarificationNewQuestion"
          />
          <div
            class="input-wrapper"
            :class="{
              'input-wrapper--clarifying': pendingClarificationRecord,
              'input-wrapper--time-clarifying': isTimeRangeClarificationPending,
            }"
            @click="clickInput"
          >
          <div v-if="(isCompletePage || selectAssistantDs) && !pendingClarificationRecord" class="datasource">
          ...
          <RecentQuestionsBar
            v-if="currentChat.records?.length && !pendingClarificationRecord"
            :current-chat="currentChat"
            @pick="onRecentQuestionPick"
          />
          <div
            v-if="computedMessages.length > 0 && currentChat.datasource && !pendingClarificationRecord"
...
import ClarificationBlock from '@/views/chat/ClarificationBlock.vue'
import RecentQuestionsBar from '@/views/chat/RecentQuestionsBar.vue'
...
function onRecentQuestionPick(question: string) {
  inputMessage.value = question
}

const pendingClarificationRecord = computed(() => {
  const records = currentChat.value?.records || []
  for (let i = records.length - 1; i >= 0; i--) {
    const r = records[i]
    if (r?.clarification?.current && !r.clarification_resolved && !r.clarification_abandoned) {
      return r
    }
  }
  return null
...
const isTimeRangeClarificationPending = computed(() => {
  const cur = pendingClarificationRecord.value?.clarification?.current
  return !!(cur && (cur.factor_type === 'time_range' || cur.factor_key === 'time_range'))
})

const inputPlaceholder = computed(() => {
  if (isTimeRangeClarificationPending.value) {
    return t('qa.clarification_time_use_picker_hint')
  }
  if (pendingClarificationRecord.value) {
    return t('qa.clarification_input_placeholder')
  }
  return t('qa.question_placeholder')
})

const forceNewQuestion = ref(false)

function findChartAnswerRef(recordId?: number) {
  if (!recordId || !chartAnswerRef.value) return null
  const refs = chartAnswerRef.value instanceof Array ? chartAnswerRef.value : [chartAnswerRef.value]
  for (const ref of refs) {
    if (!ref?.index) continue
    const i = ref.index()
    if (currentChat.value.records[i]?.id === recordId) {
      return ref
    }
  }
  return null
...
async function invokeClarify(
  record: ChatRecord,
  payload: {
    selection?: { field: string; label: string; date_start?: string; date_end?: string }
    free_text?: string
  }
) {
  if (!record?.id) return
  ...
  try {
    await nextTick()
    const ref = findChartAnswerRef(record.id)
    if (!ref) {
      ElMessage.error(t('qa.clarification_resume_failed'))
      return
    }
    await ref.clarifyMessage(payload)
  } catch (e) {
...
async function onClarificationSelect(candidate: {
  field: string
  label: string
  date_start?: string
  date_end?: string
}) {
  const pending = pendingClarificationRecord.value
  if (!pending?.id) return
  if (pending.clarification) {
    pending.clarification = { ...pending.clarification, current: null }
  }
  await invokeClarify(pending, { selection: candidate })
}

async function onClarificationNewQuestion() {
  const pending = pendingClarificationRecord.value
  if (pending?.id) {
    await questionApi.clarifyAbandon(pending.id)
    pending.clarification_abandoned = true
    pending.clarification = { ...pending.clarification, current: null }
  }
  forceNewQuestion.value = true
  ElMessage.info(t('qa.clarification_new_question_hint'))
}
...
const sendMessage = async (
  ...
  if (pending?.id && !regenerate_record_id && !forceNewQuestion.value) {
    const cur = pending.clarification?.current
    if (cur?.factor_type === 'time_range' || cur?.factor_key === 'time_range') {
      ElMessage.warning(t('qa.clarification_time_use_picker_hint'))
      return
    }
    inputMessage.value = ''
    await invokeClarify(pending, { free_text: text })
    return
  }
  const sendAsNewQuestion = forceNewQuestion.value
  forceNewQuestion.value = false
    ...
    if (chartAnswerRef.value) {
      if (chartAnswerRef.value instanceof Array) {
        for (let i = 0; i < chartAnswerRef.value.length; i++) {
          ...
          if (index === _index) {
            await chartAnswerRef.value[i].sendMessage({ new_question: sendAsNewQuestion })
            break
          }
        }
      } else {
        await chartAnswerRef.value.sendMessage({ new_question: sendAsNewQuestion })
      }
    }
  })
}
...
.chat-container {
  ...
  .chat-footer {
    ...
    .input-wrapper {
      ...
      &.input-wrapper--time-clarifying .input-area :deep(.ed-textarea__inner) {
        cursor: not-allowed;
        opacity: 0.55;
      }

      &.input-wrapper--clarifying .input-area :deep(.ed-textarea__inner) {
        padding-top: 12px;
      }

      &:not(.input-wrapper--clarifying) :deep(.recent-questions-bar) {
        margin-top: 40px;
      }
